...
## v11.11 (2026-09-17) — 探活脚本做成跨 provider 共用

### 🔴 起因：Paseo 不透传 HTTP 状态码

另一条会话用 `codebuddy/hy3` 派活，agent **不干活没反应**。Paseo 侧只看得到
「新会话零产出（`updateCount==1`）」或「turn 到头唤不醒（`activeTurn:null`）」——
**形态与 provider 稳定性故障一模一样**，而两者处置**方向相反**：
稳定性问题换 provider，配额问题换模型或等重置。

实测代价：把它误诊成「codebuddy 新会话建不起来」，据此 kill 了两个**有产出**的 agent，
换来两个零产出的。真因是 `hy3` 撞 429 —— 一条 CLI 探活就能看见，
**报错正文里连重置时间都给了**（`2026-09-17 18:43:13 UTC+8`）。

### ⭐ 把 `cb-probe.sh` 泛化成 `probe-models.sh`

用户要求做成共用。核心是**分流** —— 不同 provider 拿得到的证据强度不同，⛔ 别一套办法打天下：

| provider | 手段 | 能拿到什么 |
|---|---|---|
| `codebuddy-code` | cb CLI | ⚠️ 也不给状态码，只能判 stdout 是不是 JSON；但 429 正文带重置时间 |
| `volcengine-*` / `bailian-*` | **直连端点** | ⭐ **真实状态码** ⇒ `429`/`403`/`404` 三态分得开 |
| `github-copilot` | pi | 没有可直连的 key ⇒ 判正文 |

`cb-probe.sh` 保留为**兼容 shim**（SKILL.md 与既有笔记都引用这个名字）。

### 🔴 退出码三档，且 `2` 带样本量门槛

用户原话：「**某个模型失败其他模型可用都是正常的**」⇒ 这条必须落进退出码，⛔ 不能只写在注释里：

| exit | 含义 |
|---|---|
| `0` | 全部可用 |
| `1` | **部分**不可用 ⇒ ⚠️ **正常状态**，换模型/换池即可 |
| `2` | **全部**不可用**且样本 ≥2** ⇒ 才怀疑通道 |

⭐ **`2` 的样本量门槛是第二版才加的** —— 第一版只探 1 个模型失败就打印
「全部不可用 ⇒ 该怀疑通道」。n=1 时「全部失败」就是「这一个失败」，⛔ 推不出通道问题。
📌 与同文件那条判据同源：**断言「X 类不可用」前必须测过该类里多个实例**。

### 实跑证据（⛔ 不是「写出来了」）

```
❌ codebuddy-code        hy3                   429 …将在 2026-09-17 18:43:13 UTC+8 重置
✅ codebuddy-code        deepseek-v4.1-flash   返回 JSON
✅ volcengine-agent-plan deepseek-v4.1-flash   HTTP 200 回显 deepseek-v4-1-flash
✅ bailian-token-plan    deepseek-v4.1-flash   HTTP 200 回显 deepseek-v4.1-flash
⇒ exit=1（部分不可用）
```
四条退出路径逐个验过：n=1 失败→1 · n=2 全失败→2 · 混合→1 · 全可用→0。

⭐ 顺带确认 **百炼额度已恢复**（09-16 09:32 UTC 那次重置已过）⇒ T3 重新有落点。
⚠️ 它是**一周**配额，⛔ 不是月配额，会再次耗尽。

### ⚠️ 写脚本时又踩了自己记过的坑

`echo "全部不可用（$bad）"` —— **中文全角括号紧跟变量**，被并进变量名，
`set -u` 下直接炸 `bad�: unbound variable`。
📌 memory 里「bash 集成踩坑 §中文紧跟变量必须 `${VAR}`」写着，还是犯了。

### 验证

`consistency-check` ✅ · `pipeline-test` **98** 用例 ✅ · `coverage-check` §2 **210 行 100%** ✅
⚠️ 交叉审查按 §2.6 被拦（`--due` exit 79）⇒ 与前三次一起，合并进集成分支时审**整批**。
